...
app.get("/profile", function (req, res, next) {
	...
	axiosBase.all([
		...
		let resData = {
			...
			homepage: _.get(res1, 'data.homepage') ? _.get(res1, 'data.homepage') : '',
	...
	axiosBase.all([
		...
		resData = _.map(resData, (dt) => {
			return {
				...
				discount_price: _.get(dt, "estimate.estimate_status") === CONST.estimate_status.OFFICIAL.value ? _.get(dt, "estimate.discount_price") : null,
			}
		})
		res.send(resData);
	})).catch(function (error) {
