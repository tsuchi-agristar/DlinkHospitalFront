...
export default {
	...
	computed: {
		...
		inputDiff () {
			return (
				this.pristine_channel_dt.start_at === this.channel_dt.start_at &&
				this.pristine_channel_dt.organization_id === this.channel_dt.organization_id
			)
		},
	},
	async asyncData ( {app, params} ) {
		...
		return {
			...
			pristine_channel_dt: {
				start_at: _.cloneDeep(moment(_.get(channel.data, "event.start_at")).format("YYYY-MM-DD HH:mm:00")),
				organization_id: _.cloneDeep(_.first(_.filter(_.get(channel.data, "event_member"), (dt) => dt.member_role === CONST.member_role.OTHER.value)).organization.organization_id)
			}
		}
	},
	mounted () {
